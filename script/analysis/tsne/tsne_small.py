...
def get_num_layers(model):
    """Return the number of decoder layers for a HF CausalLM or RosettaModel."""
    if hasattr(model, "config") and hasattr(model.config, "num_hidden_layers"):
        return int(model.config.num_hidden_layers)
    if hasattr(model, "model_list") and hasattr(model, "base_model_idx"):
        ...
        if hasattr(base_model, "config") and hasattr(base_model.config, "num_hidden_layers"):
            return int(base_model.config.num_hidden_layers)
    raise ValueError("Cannot determine num_hidden_layers from the given model.")


def validate_layer_idx(model, layer_idx, model_name):
    """Fail early with a clear message when an offset maps outside the model depth."""
    num_layers = get_num_layers(model)
    if not (0 <= layer_idx < num_layers):
        raise ValueError(
            f"Invalid layer_idx={layer_idx} for {model_name}: "
            f"model has {num_layers} layers, valid range is [0, {num_layers - 1}]."
        )


def print_model_cache_shape(model, model_name):
    """Print architecture values relevant to KV-cache dimensionality and GQA ratio."""
    cfg = model.config if hasattr(model, "config") else model.model_list[model.base_model_idx].config
    num_layers = getattr(cfg, "num_hidden_layers", "unknown")
    hidden_size = getattr(cfg, "hidden_size", "unknown")
    num_heads = getattr(cfg, "num_attention_heads", "unknown")
    num_kv_heads = getattr(cfg, "num_key_value_heads", num_heads)
    head_dim = getattr(cfg, "head_dim", None)
    if head_dim is None and isinstance(hidden_size, int) and isinstance(num_heads, int):
        head_dim = hidden_size // num_heads
    flat_k_dim = num_kv_heads * head_dim if isinstance(num_kv_heads, int) and isinstance(head_dim, int) else "unknown"
    gqa_ratio = num_heads / num_kv_heads if isinstance(num_heads, int) and isinstance(num_kv_heads, int) else "unknown"
    print(
        f"  {model_name} config: layers={num_layers}, hidden_size={hidden_size}, "
        f"attn_heads={num_heads}, kv_heads={num_kv_heads}, head_dim={head_dim}, "
        f"flat_k_dim={flat_k_dim}, gqa_ratio={gqa_ratio}"
    )
...
def extract_fused_k_cache(model, tokenizer, dataset, layer_idx, num_samples=10):
    validate_layer_idx(model, layer_idx, "Rosetta/base")
    """
    ...
    for i, sample in enumerate(tqdm(subset, desc=f"Fused Layer {layer_idx}")):
        ...
        with torch.no_grad():
            ...
            if (model.base_model_idx in model.projector_dict and 
                ...
                target_slice = fused_key[:, :, -new_length:, :]
                if proj_key.shape != target_slice.shape:
                    raise ValueError(
                        "Projected K-cache shape mismatch before fusion: "
                        f"proj_key={tuple(proj_key.shape)} vs base_target={tuple(target_slice.shape)}. "
                        "This usually means the loaded projector checkpoint was trained for a different "
                        "base/teacher pair, hidden size, head_dim, or KV-head layout."
                    )
                fused_key[:, :, -new_length:, :] = proj_key
                del proj_key, target_slice
...
def extract_k_cache(model, tokenizer, dataset, layer_idx, num_samples=10):
    ...
    validate_layer_idx(model, layer_idx, getattr(model.config, "name_or_path", "standard model"))
    all_values = []
    subset = [dataset[i] for i in range(0, min(num_samples, len(dataset)))]
...
def align_dimensions(all_embeddings, target_dim=None):
    ...
    flattened KV cache dimensions. For Qwen3-0.6B and Qwen3-4B, both are 8 KV heads × 128 head_dim = 1024.
...
def compute_divergences(k_cache_per_model, layers_to_analyze, layer_idx_offset_list,
    ...
    model_names order: [Rosetta, Qwen3-0.6B, Qwen3-4B]
    Pairs measured (base=Qwen3-0.6B perspective):
      - 0.6B vs Rosetta
      - 0.6B vs 4B
      - Rosetta vs 4B
    """
    ...
    base_idx = idx_map.get('Qwen3-0.6B')
    teacher_idx = idx_map.get('Qwen3-4B')
    
    if any(v is None for v in [r_idx, base_idx, teacher_idx]):
        print("Warning: Cannot compute divergences - need all 3 models (Rosetta, Qwen3-0.6B, Qwen3-4B)")
        return
    ...
    for layer_idx in layers_to_analyze:
        ...
        teacher_actual = layer_idx + layer_idx_offset_list[model_names.index('Qwen3-4B')]
        ...
        dist_0_6b = kv_to_distribution(aligned[1])
        dist_4b = kv_to_distribution(aligned[2])
        
        kl_0_6b_r = kl_divergence(dist_0_6b, dist_r)
        kl_0_6b_4b = kl_divergence(dist_0_6b, dist_4b)
        kl_r_4b = kl_divergence(dist_r, dist_4b)
        
        js_0_6b_r = js_divergence(dist_0_6b, dist_r)
        js_0_6b_4b = js_divergence(dist_0_6b, dist_4b)
        js_r_4b = js_divergence(dist_r, dist_4b)
        
        row = {
            'layer_mapping': mapping_str,
            'KL_0_6B-R': f"{kl_0_6b_r:.6f}",
            'KL_0_6B-4B': f"{kl_0_6b_4b:.6f}",
            'KL_R-4B': f"{kl_r_4b:.6f}",
            'JS_0_6B-R': f"{js_0_6b_r:.6f}",
            'JS_0_6B-4B': f"{js_0_6b_4b:.6f}",
            'JS_R-4B': f"{js_r_4b:.6f}",
        }
        rows.append(row)
        print(f"  Layer {mapping_str}: KL(0.6B||R)={kl_0_6b_r:.4f}  KL(0.6B||4B)={kl_0_6b_4b:.4f}  "
              f"KL(R||4B)={kl_r_4b:.4f}  JS(0.6B,R)={js_0_6b_r:.4f}  JS(0.6B,4B)={js_0_6b_4b:.4f}  "
              f"JS(R,4B)={js_r_4b:.4f}")
    
    # Write CSV
    fieldnames = ['layer_mapping', 'KL_0_6B-R', 'KL_0_6B-4B', 'KL_R-4B', 'JS_0_6B-R', 'JS_0_6B-4B', 'JS_R-4B']
...
def plot_sequence_correspondence_lines(tsne_result, sequence_indices, model_names, label):
    ...
    # Qwen3-0.6B (base) <-> Rosetta correspondence
    ...
    # Rosetta <-> Qwen3-4B (teacher) correspondence
...
def main(args):
    ...
    # Analyze the last 7 base layers of Qwen3-0.6B.
    # Qwen3-0.6B has 28 layers -> base layers 21-27.
    # Qwen3-4B has 36 layers, so equivalent-depth teacher layers are 29-35.
    layers_to_analyze = [21, 22, 23, 24, 25, 26, 27]
    # Offsets: [Rosetta fused/base, Qwen3-0.6B base, Qwen3-4B teacher]
    ...
    for model_idx, (model_path, layer_offset) in enumerate(zip(args['models'], layer_idx_offset_list)):
        ...
        if "Rosetta" in model_path:
            model, tokenizer = load_rosetta_model(args['checkpoint_dir'])
        else:
            model, tokenizer = load_qwen_model(model_path)
        model.eval()
        print_model_cache_shape(model, model_path)
...
if __name__ == "__main__":
    ...
    python tsne.py --models Rosetta Qwen3-0.6B         # Specify models to analyze
    ...
    parser.add_argument('--output_dir', type=str, default="tsne_outputs_openbookqa_qwen3_0_6b_qwen3_4b",
                       help='Output directory for t-SNE plots')
    parser.add_argument('--checkpoint_dir', type=str,
                       default="local/checkpoints/C2C_Fuser/qwen3_0.6b+qwen3_4b_Fuser/final",
                       help='Rosetta/C2C fuser checkpoint directory for Qwen3-0.6B + Qwen3-4B')
    # Changed: Default models to match Fuser config (Qwen3-0.6B + Qwen3-4B)
